	...
	for layer in range(0, len(H)):
		...
		# 池化层
			...
			for m in range(0, input_num):
				# 对于每个input_map进行计算
				# print(input_img[m])
				pool_m = np.zeros((int(input_size[0]//Pm), int(input_size[1]//Pn)))
				for i in range(0, int(input_size[0]//Pm)):
					for j in range(0, int(input_size[1]//Pn)):
						pool_m[i, j] = np.max(input_img[m][Pm*i:Pm*i+Pm, Pn*j:Pn*j+Pn])
...
if __name__ == '__main__':
	...
	dict_para_addr, dict_para_val, inst_set, _ = generate_npu_inst.generate_npu_inst(H, value_enable=True)
	#print(inst_set)
	# 首先存储好NPU的指令
	...
	sample_image, sample_label, _, _ = load_all_mnist_samples()
	# 不需要额外的归一化操作
